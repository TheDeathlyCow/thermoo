...
/// Identical to the Fabric API version of this class, but enabled for multiloader.
final class TestAnnotationLocator {
    private static final String ENTRYPOINT_KEY = "thermoo:gametest";
    private static final Logger LOGGER = LoggerFactory.getLogger(TestAnnotationLocator.class);

    private final YumiMods yumiMods;

    private List<TestMethod> testMethods = null;

    TestAnnotationLocator(YumiMods yumiMods) {
        this.yumiMods = yumiMods;
    }
    ...
    public List<TestMethod> getTestMethods() {
        ...
        List<EntrypointContainer<Object>> entrypointContainers = yumiMods.getEntrypoints(ENTRYPOINT_KEY, Object.class);
    ...
    private List<TestMethod> findMagicMethods(EntrypointContainer<Object> entrypoint) {
        Class<?> testClass = entrypoint.value().getClass();
    ...
    public record TestMethod(Method method, GameTest gameTest, EntrypointContainer<Object> entrypoint) {
        Identifier identifier() {
            String name = camelToSnake(entrypoint.value().getClass().getSimpleName() + "_" + method.getName());
            return Identifier.fromNamespaceAndPath(entrypoint.mod().id(), name);
        }

        Consumer<GameTestHelper> testFunction() {
            return context -> {
                Object instance = entrypoint.value();
