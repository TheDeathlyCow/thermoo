...
public sealed interface EnvironmentController permits DefaultEnvironmentController, EnvironmentControllerDecorator {
    ...
    /**
     * Gets the amount of warmth generated by the 'hot floor' state.
     * <p>
     * Hot floor is different from {@link #getHeatFromBlockState(BlockState)}, as it ONLY applies to entities stepping on
     * the block - it does not affect the area around the block. For example, {@link net.minecraft.block.Blocks#MAGMA_BLOCK}
     * would be a hot floor, but it does not provide area heat.
     *
     * @param state The state of the hot floor.
     * @return Returns the heat to apply each tick to entities standing on the block state
     */
    int getHotFloorWarmth(BlockState state);
